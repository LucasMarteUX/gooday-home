...
export function StoryViewer({ vm }: Props) {
  ...
  return (
    <div role="dialog" aria-label="Story" className="fixed inset-0 z-[80] animate-[gd-fade_180ms_ease] bg-[#080B0E]">
      ...
      <div className="absolute left-0 right-0 top-[max(12px,env(safe-area-inset-top))] flex gap-[5px] px-4 py-2">
      ...
      <div className="absolute left-4 right-4 top-[calc(28px+env(safe-area-inset-top))] flex items-center gap-2.5">
        <img
          src={vm.story.av}
          alt=""
      ...
      <div className="absolute bottom-[max(26px,calc(16px+env(safe-area-inset-bottom)))] left-4 right-4 flex items-center gap-2.5">
